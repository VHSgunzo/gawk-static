...
[ "$gawk_version" == "latest" ] && \
  gawk_version="$(curl -s https://ftp.gnu.org/gnu/gawk/|tac|\
                       grep -om1 'gawk-.*\.tar\.xz'|cut -d'>' -f2|sed 's|gawk-||g;s|.tar.xz||g')"
...
# download tarballs
echo "= downloading gawk v${gawk_version}"
curl -LO https://ftp.gnu.org/gnu/gawk/gawk-${gawk_version}.tar.gz
...
tar --xz -acf gawk-static-v${gawk_version}-${platform_arch}.tar.xz release
